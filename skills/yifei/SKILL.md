...
## 取数前:凭据检查与引导
1. 先静默检查配置: `yifei config show`。
2. 若返回「配置文件不存在」→ **不要让用户自己敲命令**,而是友好地逐项向用户索取连接信息(给默认值,只有密码必填,其余可回车用默认):
   - 主机 host(默认 `localhost`)、端口 port(默认 `1433`)
   - 用户名 user(默认 `sa`)、**密码 password(必填)**、库名 database(默认 `YDSTEST`)
3. 拿到信息后由 **Agent 代为执行**(密码不要在回复里回显):
   `yifei config init --host <h> --port <p> --user <u> --password <pw> --database <db>`
4. 回读确认: `yifei config show`(密码自动脱敏),告诉用户「已配置完成,可以开始查询」。
5. 之后若查询报「数据库连接失败」,提示可能是密码/主机/网络问题,请用户更正后由 Agent 重新 `config init`,不要让用户手动操作。
